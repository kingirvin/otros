...
$( document ).ready(function() {
    ...
    tabla = $("#t_documentos").DataTable({
        "processing": true,
        "serverSide": true,
        "pageLength": 50,
        "order": [],
        "ajax": {
            "url":  default_server+"/json/tipodocumentos",
            "type": "GET",
            "data": function ( d ) { },
        ...
        "columns": [            
            ...
            { "data": "documentos_count", "searchable": false, className: "w-1",
                render: function ( data, type, full ) {       
                    return data;
                }        
            },         
...
function guardar() 
{
    if(validar('#form_editar'))
    {
        ...
        else
        {
            $.ajax({
                type: "PUT",
                url: default_server+"/json/tipodocumentos/"+editItem,
                data: {                   
                    nombre: $("#nombre").val(),
                    abreviatura: $("#abreviatura").val(),
                    estado: ($("#estado").is(':checked') ? 1 : 0 )
...
function eliminar(iditem) {
    if(confirm('Esta seguro que desea eliminar?'))
    {
        $.ajax({
            type: "DELETE",
            url: default_server+"/json/tipodocumentos/"+iditem,        
            success: function(result){  
                alerta(result.message, true); 
                tabla.ajax.reload();
            },
