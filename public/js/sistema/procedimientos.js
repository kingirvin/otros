...
$( document ).ready(function() {
    ...
    tabla = $("#t_procedimiento").DataTable({
        "processing": true,
        "serverSide": true,
        "pageLength": 50,
        "order": [],
        "ajax": {
            "url":  default_server+"/json/procedimientos",
            "type": "GET",
            "data": function ( d ) {
        ...
        "columns": [     
            { "data": "codigo", className: "w-1" },
            { "data": "titulo",
                render: function ( data, type, full ) {                      
                    return '<div title="'+data+'">'+textoMax(data,35)+'</div>';
                }        
            },
            { "data": "presentar.nombre",
                render: function ( data, type, full ) {                      
                    return '<div title="'+data+'">'+textoMax(data,35)+'</div>';
                }        
            },  
            { "data": "plazo", "searchable": false, className: "w-1",
                render: function ( data, type, full ) {       
                    return data+" dias";
                }        
            }, 
            { "data": null, "searchable": false, "orderable": false, className: "text-center w-1",
                render: function ( data, type, full ) {       
                    return full.pasos.length;
                }        
            },                   
            ...
            { 
                ...
                "searchable": false, "orderable": false, className: "w-1",
                    ...
                    var res = '<div class="btn-list flex-nowrap">'+
                                '<a href="'+default_server+'/admin/sistema/documental/procedimientos/'+full.id+'/pasos" class="btn btn-info btn-icon" title="PASOS">'+
                                ...
                                '</a>'+
                                '<a href="'+default_server+'/admin/sistema/documental/procedimientos/'+full.id+'/modificar" class="btn btn-white btn-icon" title="MODIFICAR">'+
...
function eliminar(iditem) {
    if(confirm('Esta seguro que desea eliminar?'))
    {
        $.ajax({
            type: "DELETE",
            url: default_server+"/json/procedimientos/"+iditem,        
