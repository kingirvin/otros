...
$( document ).ready(function() {
    ...
    tabla = $("#t_sedes").DataTable({
        "processing": true,
        "serverSide": true,
        "pageLength": 50,
        "order": [],
        "ajax": {
            "url":  default_server+"/json/sedes",
            "type": "GET",
            "data": function ( d ) { },
        ...
        "columns": [            
            ...
            { "data": "dependencias", "orderable": false, "searchable": false, className: "w-1",
                render: function ( data, type, full ) {       
                    return data.length;
                }        
            },         
...
function guardar() 
{
    if(validar('#form_editar'))
    {
        ...
        else
        {
            $.ajax({
                type: "PUT",
                url: default_server+"/json/sedes/"+editItem,
                data: {                   
                    nombre: $("#nombre").val(),
                    abreviatura: $("#abreviatura").val(),
                    direccion: $("#direccion").val(),
...
function eliminar(iditem) {
    if(confirm('Esta seguro que desea eliminar?'))
    {
        $.ajax({
            type: "DELETE",
            url: default_server+"/json/sedes/"+iditem,        
            success: function(result){  
                alerta(result.message, true); 
                tabla.ajax.reload();
            },
